  ...
  static show(modal, row, target, list, filesToSave) {
    ...
    if (target && target.closest('li.file-element')) {
      const fileNode = target
        .closest('li.file-element')
        .querySelector('.preview, .preview-image')
        .cloneNode(true);
      fileNode.id = 'preview-for-download';
      modal
        .querySelector('.modal-file-container')
        .insertAdjacentElement('afterbegin', fileNode);
    }
    if (target && target.classList.contains('new')) {
      this.column = target
        .closest('.column')
        .querySelector('.column-container').id;
    }
    ...
    if (list && row) {
      const filesToRender = list.find(
        (item) => item.id === row.getAttribute('data-id'),
      ).files;
      // Отрисовываем файлы
    ...
    if (rowId) {
      ...
      try {
        ...
        await Storage.request(
          'update',
          JSON.stringify({ id: rowId, name, files: filesToSave }),
        );
      } catch (error) {
        return { error };
      }
      ...
      const order = () =>
        list ? list.filter((item) => item.column === column).length + 1 : 1;
      const newbie = {
        id: id(),
        order: order(),
        column,
