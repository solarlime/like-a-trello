...
export default class Utils {
  ...
  static renderSpace(event, drag, theSameItem = 0) {
    ...
    const spaceToRemove = drag?.closest('.column-container')
      ? drag.closest('.column-container').querySelector('.column-space')
      : null;
    ...
    if (pointItem) {
      ...
      if (!theSameItem) {
        if (
          event.pageY >
          window.scrollY +
            pointItem.getBoundingClientRect().top +
            pointItem.offsetHeight / 2
        ) {
  ...
  static makeFile(fileObject) {
    return fetch(fileObject.link)
      ...
      .then(
        (result) =>
          new File([result], fileObject.name, { type: fileObject.type }),
      );
  }
  ...
  static renderFiles(modal, file) {
    ...
    if (
      !modal.classList.contains('modal-add-update') &&
      !modal.classList.contains('column-item-files')
    ) {
      return null;
    }
    ...
    const svg =
      '<svg class="file-remove" viewBox="0 0 329.26933 329" xmlns="http://www.w3.org/2000/svg">' +
    ...
    if (!file.type.includes('image')) {
      ...
      previewItem.textContent = modal.classList.contains('modal-add-update')
        ? fileType
        : file.name;
  ...
  static render(item) {
    ...
    newRow.innerHTML =
      '<div class="column-item-paragraph">\n' +
  ...
  static reset() {
    ...
    Array.from(document.querySelector('.files').children).forEach((item) =>
      item.remove(),
    );
    const file = Array.from(
      document.querySelector('.modal-file-container').children,
    ).find(
      (item) =>
        item.classList.contains('preview') ||
        item.classList.contains('preview-image'),
    );
    const space = document.querySelector('.column-space');
    if (space) {
      space.remove();
  ...
  static checkSize(quota, string) {
    return new Promise((resolve, reject) => {
      ...
      if (quota - used < quota / 10) {
        reject(
          new Error(
            'Not saved! The free space is almost run out. Delete some unnecessary data.',
          ),
        );
      } else {
        resolve();
